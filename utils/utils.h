...
namespace utils 
{
    ...
    struct Box
    {
        ...
        Box(float left, float top, float right, float bottom, float confidence, int label) :
            left(left), top(top), right(right), bottom(bottom), confidence(confidence), label(label), track_id(-1) {}
    ...
    byte_track::Object convertBoxToByteTrackObject(const Box& box);
    std::vector<byte_track::Object> convertBoxesToByteTrackObjects(const std::vector<Box>);

    void setTrackIdToBoxes(std::vector<Box>&, const std::vector<byte_track::BYTETracker::STrackPtr>);
    
    void show(const std::vector<std::vector<Box>>& objectss, const std::vector<std::string>& classNames,
        const int& cvDelayTime, std::vector<cv::Mat>& imgsBatch, cpp_ai_utils::CppAiHelper&, const std::string& queueName = "");
